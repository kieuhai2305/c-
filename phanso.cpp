...
void rutgon(phanso &ps)
{
	...
	ps.ms=ps.ms/c;
		...
		{
			//tinh tong phan so: p+ phan sohien hanh this
			//p.ts/p.ms +this.ts/this.ms
			//a/b+c/d=(ad+bc)/bd
			phanso tong;
...
p2.xuat();
cout<<"\ntong hai phan so : ";
phanso tong=p1.congps(p2); 
tong.xuat() ;

cout<<"\nhieu hai phan so : ";
phanso hieu=p1.trups(p2);
hieu.xuat();

cout<<"\ntich hai phan so : ";
phanso tich=p1.nhanps(p2);
tich.xuat();

cout<<"\nthuong hai phan so : ";
phanso thuong=p2.chaips(p1);
thuong.xuat();
